...
from .models import Department_company, Project, Salary,Leave,Department


class salary(admin.ModelAdmin):
    list_display=['salary_id','emp_id','paid_date','month','salary','created_at']
admin.site.register(Salary,salary)    

class leave(admin.ModelAdmin):
    list_display=['leave_id','emp_id','from_date','to_date','leave_type','subject','reason','apply_date','created_at','modified_at']
